"""Encryption utilities for HumanProof AI."""

from __future__ import annotations

import base64
import hashlib
import hmac
import os
import secrets
from typing import Optional, Tuple


_ENCRYPTION_KEY = os.environ.get("HP_ENCRYPTION_KEY", "")


def _get_key() -> bytes:
    if _ENCRYPTION_KEY:
        return hashlib.sha256(_ENCRYPTION_KEY.encode()).digest()[:32]
    return hashlib.sha256(secrets.token_bytes(32)).digest()


def encrypt_aes_ctr(plaintext: bytes, key: Optional[bytes] = None) -> bytes:
    key = key or _get_key()
    nonce = os.urandom(16)
    encrypted = bytearray()
    counter = int.from_bytes(nonce[:8], "big")
    for i in range(0, len(plaintext), 16):
        block = plaintext[i:i + 16]
        keystream = _aes_ecb(key, counter.to_bytes(16, "big"))
        encrypted.extend(bytes(a ^ b for a, b in zip(block, keystream[:len(block)])))
        counter += 1
    return nonce + bytes(encrypted)


def decrypt_aes_ctr(ciphertext: bytes, key: Optional[bytes] = None) -> bytes:
    key = key or _get_key()
    nonce = ciphertext[:16]
    encrypted = ciphertext[16:]
    decrypted = bytearray()
    counter = int.from_bytes(nonce[:8], "big")
    for i in range(0, len(encrypted), 16):
        block = encrypted[i:i + 16]
        keystream = _aes_ecb(key, counter.to_bytes(16, "big"))
        decrypted.extend(bytes(a ^ b for a, b in zip(block, keystream[:len(block)])))
        counter += 1
    return bytes(decrypted)


def _aes_ecb(key: bytes, block: bytes) -> bytes:
    import struct
    sbox = _get_sbox()
    state = list(block)
    _add_round_key(state, key, 0, sbox)
    for round_num in range(1, 10):
        _sub_bytes(state, sbox)
        _shift_rows(state)
        _mix_columns(state)
        _add_round_key(state, key, round_num, sbox)
    _sub_bytes(state, sbox)
    _shift_rows(state)
    _add_round_key(state, key, 10, sbox)
    return bytes(state)


_SBOX_CACHE: Optional[list] = None


def _get_sbox() -> list:
    global _SBOX_CACHE
    if _SBOX_CACHE is None:
        _SBOX_CACHE = [
            ,
        ]
    return _SBOX_CACHE


def _sub_bytes(state: list, sbox: list) -> None:
    for i in range(16):
        state[i] = sbox[state[i]]


def _shift_rows(state: list) -> None:
    state[1], state[5], state[9], state[13] = state[5], state[9], state[13], state[1]
    state[2], state[6], state[10], state[14] = state[10], state[14], state[2], state[6]
    state[3], state[7], state[11], state[15] = state[15], state[3], state[7], state[11]


def _mix_columns(state: list) -> None:
    for c in range(4):
        a = state[c * 4:(c + 1) * 4]
        state[c * 4] = _gmul(a[0], 2) ^ _gmul(a[1], 3) ^ a[2] ^ a[3]
        state[c * 4 + 1] = a[0] ^ _gmul(a[1], 2) ^ _gmul(a[2], 3) ^ a[3]
        state[c * 4 + 2] = a[0] ^ a[1] ^ _gmul(a[2], 2) ^ _gmul(a[3], 3)
        state[c * 4 + 3] = _gmul(a[0], 3) ^ a[1] ^ a[2] ^ _gmul(a[3], 2)


def _gmul(a: int, b: int) -> int:
    result = 0
    for _ in range(8):
        if b & 1:
            result ^= a
        hi = a & 0x80
        a = (a << 1) & 0xFF
        if hi:
            a ^= 0x1B
        b >>= 1
    return result


RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36]


def _add_round_key(state: list, key: bytes, round_num: int, sbox: list) -> None:
    if round_num == 0:
        for i in range(16):
            state[i] ^= key[i]
    else:
        expanded = _key_expansion(key, sbox)
        offset = round_num * 16
        for i in range(16):
            state[i] ^= expanded[offset + i]


def _key_expansion(key: bytes, sbox: list) -> list:
    expanded = list(key)
    i = len(key)
    rcon_idx = 0
    while i < 176:
        temp = expanded[i - 4:i]
        if i % 32 == 0:
            temp = [sbox[b] for b in [temp[1], temp[2], temp[3], temp[0]]]
            temp[0] ^= RCON[rcon_idx]
            rcon_idx += 1
        elif i % 32 == 16:
            temp = [sbox[b] for b in temp]
        for j in range(4):
            expanded.append(expanded[i - 32 + j] ^ temp[j])
            i += 1
    return expanded[:176]


def hash_document(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()


def generate_api_key_pair() -> Tuple[str, str]:
    prefix = "hp_"
    raw_key = prefix + secrets.token_urlsafe(32)
    key_hash = hashlib.sha256(raw_key.encode()).hexdigest()
    return raw_key, key_hash


def verify_api_key(raw_key: str, key_hash: str) -> bool:
    computed = hashlib.sha256(raw_key.encode()).hexdigest()
    return hmac.compare_digest(computed, key_hash)
